...
def test_evidence_autopilot_research_builder_returns_auditable_tasks() -> None:
    ...
    assert response.targetLabel == "广东 2026 华南理工示例校 智能制造与数据工程"
    assert len(response.tasks) >= 8
    assert len(response.searchQueries) >= 8
    assert len(response.evidenceCards) == len(response.tasks)
    assert "不会承诺录取" in response.claimBoundary
    assert "人工采集" in response.claimBoundary
...
def test_evidence_autopilot_research_endpoint_is_wired_to_fastapi() -> None:
    ...
    assert payload["targetLabel"] == "广东 2026 华南理工示例校 智能制造与数据工程"
...
if __name__ == "__main__":
    test_evidence_autopilot_research_builder_returns_auditable_tasks()
