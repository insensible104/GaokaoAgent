...
const closedPreviewPath = path.join(lib, "evidenceAutopilotRealCaseOperatorClosureDeliveryPreview.ts");
...
const closedPreview = loadTsModule(closedPreviewPath, {
  "./evidenceAutopilotRealCaseOperatorClosure": operatorClosure,
});

assert.equal(typeof closedPreview.bootstrapRealCaseOperatorClosureDeliveryPreview, "function");
assert.equal(typeof closedPreview.listRealCaseOperatorClosureClientFacingArtifacts, "function");
...
const result = await closedPreview.bootstrapRealCaseOperatorClosureDeliveryPreview({
  fixture,
  caseId,
  plan,
  operatorInput,
...
assert.equal(result.protocol, "real_case_operator_closure_delivery_preview_v1");
assert.equal(result.caseId, caseId);
assert.equal(result.workflow.protocol, "real_case_operator_closure_workflow_v1");
assert.equal(result.workflow.closureReview.browser.missingP0TaskIds.length, 0);
assert.equal(result.workflow.closureReview.auditPacket.status, "requires_counter_evidence_review");
assert.equal(result.preview.case_id, caseId);
assert.equal(result.preview.manifest.status, "requires_counter_evidence_review");
assert.equal(result.preview.manifest.client_delivery.allowed, false);
assert.match(result.preview.manifest.client_delivery.blocked_reason, /counter-evidence/i);
...
assert.deepEqual(
  ...
  ["real_case_operator_closure_brief", "real_case_operator_closure_json"],
);
assert.match(result.preview.artifacts.real_case_operator_closure_brief, /employment-market/);
assert.match(result.preview.artifacts.real_case_operator_closure_brief, /requires_counter_evidence_review/);
assert.match(result.preview.artifacts.real_case_operator_closure_json, /review-job-market/);
...
const clientArtifacts = closedPreview.listRealCaseOperatorClosureClientFacingArtifacts(result.preview);
assert.deepEqual(clientArtifacts, []);

console.log("Evidence Autopilot real case operator closure delivery preview test passed");
