...
export function buildDeepEvidenceCollectionPlan(
  ...
  const tasks: DeepEvidenceTask[] = [
    {
      ...
      accessMethod: "核验官方网页或 PDF，保留发布日期、学校代码、专业组代码、计划数、选科要求和校区安排。",
      outputFields: ["来源链接", "发布日期", "学校代码", "专业组代码", "计划数", "选科要求", "校区", "原文摘录"],
      passRule: "官方字段齐全，且和机会卡目标一致，才允许进入量化定位。",
    ...
    },
    {
      ...
      accessMethod: "至少两类来源交叉核验，记录年份、最低分、最低位次、计划数变化和口径差异。",
    ...
    },
    {
      ...
      accessMethod: "查本科生是否真实进入项目、竞赛、课题组或导师团队，优先保留学生参与方式和报名入口。",
    ...
    },
    {
      ...
      accessMethod: "采样岗位，记录城市、学历门槛、技能栈、工作内容和薪资口径；半封闭平台只做合规人工采集。",
    ...
    },
    {
      ...
      accessMethod: "拆分校内保研、外校保研、考研方向和跨学科去向，记录本科阶段可准备的项目经历。",
    ...
    },
    {
      ...
      accessMethod: "按专业名称、学科门类和岗位限制查可报岗位，记录是否只能走不限专业岗位。",
    ...
    },
    {
      ...
      accessMethod: "通过微信客户端、搜一搜、文章链接或可访问搜索结果采集；不绕过登录、付费或平台限制。",
      outputFields: ["公众号名称", "文章标题", "发布日期", "涉及导师/项目/就业", "链接或截图编号", "原文摘录"],
      passRule: "公众号材料只补充官网没有的过程证据，必须和官方或学院来源双来源印证。",
  ...
  return {
    ...
    claimBoundary:
      "这不是自动抓取承诺，而是高维证据采集台账；平台型、半封闭或公众号材料必须合规采集，不能绕过登录、付费或访问限制。",
  };
}
